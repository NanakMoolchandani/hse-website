...
export default function Seatex() {
  ...
  useEffect(() => {
    ...
    async function loadAll() {
      ...
      await Promise.all(
        SEATEX_COLLECTIONS.map(async (col) => {
  ...
  const activeCol = SEATEX_COLLECTIONS.find((c) => c.handle === activeHandle)
  ...
  const filteredProducts = useMemo(() => {
    ...
    return products.filter((p) => cleanSeatexTitle(p.title).toLowerCase().includes(q))
  }, [products, searchQuery])

  return (
    <>
      <SEO
        title="Seatex Furniture in Neemuch - Authorized Dealer | Plastic Chairs, Tables"
        description="Buy Seatex furniture in Neemuch from Hari Shewa Enterprises, authorized wholesale dealer. Standard and premium plastic chairs, tables, stools, and kids furniture. Call +91 99815 16171."
        canonical="/seatex"
        keywords="Seatex furniture dealer Neemuch, Seatex plastic chairs, Seatex wholesale MP"
              ...
              <div className='h-full overflow-y-auto pt-2 pb-4 px-2'>
                ...
                <nav className='space-y-0.5'>
                  <button onClick={() => selectCollection(ALL)}
                    className={`w-full text-left px-2 py-1.5 rounded-lg text-xs transition-all duration-150 ${activeHandle === ALL ? 'bg-emerald-600 text-white font-semibold shadow-sm' : 'text-gray-600 hover:bg-white hover:text-gray-900 hover:shadow-sm'}`}>
                    <span className='flex items-center justify-between gap-2'>
                      <span className='leading-snug'>All Products</span>
                      {totalCount > 0 && <span className={`text-[11px] flex-shrink-0 font-medium ${activeHandle === ALL ? 'text-emerald-100' : 'text-gray-400'}`}>{totalCount}</span>}
                    </span>
                  </button>
                  {SEATEX_COLLECTIONS.map((col) => {
                    ...
                    return (
                      <button key={col.handle} onClick={() => selectCollection(col.handle)}
                        className={`w-full text-left px-2 py-1.5 rounded-lg text-xs transition-all duration-150 ${isActive ? 'bg-emerald-600 text-white font-semibold shadow-sm' : 'text-gray-600 hover:bg-white hover:text-gray-900 hover:shadow-sm'}`}>
                        <span className='flex items-center justify-between gap-2'>
                          <span className='leading-snug'>{col.label}</span>
                          {count !== undefined && <span className={`text-[11px] flex-shrink-0 font-medium ${isActive ? 'text-emerald-100' : 'text-gray-400'}`}>{count}</span>}
                        </span>
                      </button>
                    )
                  })}
                ...
                <div className='mt-5 p-3.5 rounded-xl bg-white border border-gray-100 shadow-sm'>
                  ...
                  <p className='text-[11px] text-gray-500 mb-3 leading-relaxed'>Wholesale rates on all Seatex products.</p>
                  <a href='https://wa.me/919981516171?text=Hi%2C%20I%20need%20wholesale%20pricing%20on%20Seatex%20furniture.'
                    className='block w-full text-center text-xs font-semibold bg-emerald-600 text-white py-2 rounded-lg hover:bg-emerald-700 transition-colors'>WhatsApp Us</a>
                </div>
              </div>
            </aside>
            <main className='flex-1 min-w-0 bg-gray-50 px-6 pt-20 md:pt-[120px] pb-8'>
              <div className='md:hidden mb-4'>
                <div className='flex gap-2 overflow-x-auto pb-2'>
                  <button onClick={() => selectCollection(ALL)}
                    className={`flex-shrink-0 px-3.5 py-1.5 rounded-full text-xs font-medium transition-all ${activeHandle === ALL ? 'bg-emerald-600 text-white shadow-sm' : 'bg-white border border-gray-200 text-gray-600'}`}>
                    All
                  </button>
                  {SEATEX_COLLECTIONS.map((col) => {
                    const isActive = col.handle === activeHandle
                    return (
                      <button key={col.handle} onClick={() => selectCollection(col.handle)}
                        className={`flex-shrink-0 px-3.5 py-1.5 rounded-full text-xs font-medium transition-all ${isActive ? 'bg-emerald-600 text-white shadow-sm' : 'bg-white border border-gray-200 text-gray-600'}`}>
                        {col.label}
                      </button>
                    )
                  })}
              ...
              <div className='flex items-center gap-4 mb-4 flex-wrap'>
                ...
                <div className='relative flex-shrink-0 w-full sm:w-56'>
                  ...
                  <input type='text' placeholder='Search products...' value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)}
                    className='w-full pl-9 pr-3 py-2 bg-white border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-emerald-300 focus:border-transparent' />
                </div>
              </div>
              ...
              {isLoading ? (
                    ...
                    return (
                      <Link key={product.id} to={`/seatex/${activeHandle === ALL ? (Object.entries(collectionProducts).find(([, ps]) => ps.some(p => p.id === product.id))?.[0] ?? SEATEX_COLLECTIONS[0].handle) : activeHandle}/${product.handle}`}
                        ...
                        <div className='p-3 border-t border-gray-50'>
                          <h4 className='text-sm font-medium text-gray-800 leading-snug line-clamp-2 mb-1.5 group-hover:text-emerald-600 transition-colors'>{title}</h4>
                          <span className='text-xs text-emerald-600 font-medium group-hover:text-emerald-700 transition-colors'>View Details →</span>
                        </div>
                      </Link>
                    )
                  })}
                ...
                <div className='rounded-xl border border-dashed border-gray-200 bg-white p-12 text-center'>
                  ...
                  <a href={`https://wa.me/919981516171?text=${encodeURIComponent(`Hi, I'm interested in Seatex ${activeCol?.label ?? 'products'}. Please share what's available.`)}`}
                ...
                <div className='rounded-xl border border-dashed border-gray-200 bg-white p-12 text-center'>
                  ...
                  <button onClick={() => setSearchQuery('')} className='text-xs text-emerald-600 hover:text-emerald-700 transition-colors'>Clear search</button>
                </div>
              )}
            </main>
          </div>
