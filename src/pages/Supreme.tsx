                  ...
                  {filteredProducts.map((product) => {
                    ...
                    return (
                      <Link key={product.id} to={`/supreme/${activeHandle === ALL ? (Object.entries(collectionProducts).find(([, ps]) => ps.some(p => p.id === product.id))?.[0] ?? SUPREME_COLLECTIONS[0].handle) : activeHandle}/${product.handle}`}
                        className='group reveal rounded-2xl bg-white border border-black/[0.06] overflow-hidden transition-[transform,box-shadow,border-color] duration-250 ease-spring hover:-translate-y-1.5 hover:border-orange-300 hover:shadow-xl'>
