                  ...
                  {SUPREME_COLLECTIONS.map((col) => {
                    ...
                    return (
                      <button key={col.handle} onClick={() => selectCollection(col.handle)}
                        className={`w-full text-left px-3 py-2 rounded-lg text-sm transition-all duration-150 ${isActive ? 'bg-orange-500 text-white font-semibold shadow-sm' : 'text-gray-600 hover:bg-white hover:text-gray-900 hover:shadow-sm'}`}>
                        <span className='flex items-center justify-between gap-2'>
