  ...
  return (
    <div
      ...
      className={cn(
        ...
        fitContent ? '' : 'w-[320px] [aspect-ratio:17/21]',
        containerClassName,
      )}
      ref={refElement}
      onPointerMove={(event) => {
    ...
    >
      {fitContent ? (
        ...
        <div className='h-full relative will-change-transform origin-center transition-transform duration-[var(--duration)] ease-[var(--easing)] delay-[var(--delay)] [transform:rotateY(var(--r-x))_rotateX(var(--r-y))] rounded-[var(--radius)] border border-slate-800 hover:[--opacity:0.6] hover:[--duration:200ms] hover:[--easing:linear] hover:filter-none overflow-hidden'>
          ...
          <div
            className='absolute inset-0 mix-blend-color-dodge opacity-[var(--opacity)] will-change-background transition-opacity [clip-path:inset(0_0_1px_0_round_var(--radius))] [background-blend-mode:hue_hue_hue_overlay] [background:var(--pattern),_var(--rainbow),_var(--diagonal),_var(--shade)]'
            style={{ ...backgroundStyle }}
          />
        </div>
      ) : (
        /* Fixed aspect-ratio: original stacked-grid layout */
